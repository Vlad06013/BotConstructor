...
import (
	...
	"github.com/Vlad06013/BotConstructor.git/domain/messageTemplates/domainsTemplate"
	"github.com/Vlad06013/BotConstructor.git/domain/messageTemplates/linksTemplates"
	"github.com/Vlad06013/BotConstructor.git/domain/messageTemplates/tariffsTemplate"
...
func findTemplate(callBack string, client telegramProfile.TelegramProfile, conn *gorm.DB, message *tgbotapi.Message) external.TextMessage {
	...
	if callBack == "start_free_period" {
		return tariffsTemplate.StartFreePeriodMessage(client, conn)
	}

	if callBack == "new_domain" {
		return domainsTemplate.NewDomainMessage(client, conn)
	}

	if callBack == "connect_domain" {
		return domainsTemplate.ConnectDomainMessage(client, conn)
	}

	if callBack == "wait_input_domain" {
		return domainsTemplate.WaitInputDomainMessage(client, conn)
	}

	if callBack == "save_input_domain" {
		return domainsTemplate.SaveInputDomainMessage(client, conn, message)
	}
	...
	if callBack == "chooseDomainToShotLink" {
		return domainsTemplate.ChooseDomainToShotLinkMessage(client, conn)
	}

	if strings.Contains(callBack, "shotLink") {
		res := strings.Split(callBack, "|")
		u64, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.WaitInputShotLinkMessage(client, conn, uint(u64))
	}

	if strings.Contains(callBack, "save_input_link") {
		res := strings.Split(callBack, "|")
		domainId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.SaveInputShotLinkMessage(client, conn, message, uint(domainId))
	}
	if callBack == "linkSettings" {
		return linksTemplates.LinkSettingsMessage(client, conn)
	}

	if callBack == "domainSettings" {
		return domainsTemplate.DomainSettingsMessage(client, conn)
	}

	if callBack == "tariffSettings" {
		return tariffsTemplate.TariffsSettingsMessage(client, conn)
	}

	if strings.Contains(callBack, "detailLink") {
		res := strings.Split(callBack, "|")
		u64, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.DetailLinkMessage(client, conn, uint(u64))
	}

	if strings.Contains(callBack, "detailDomain") {
		res := strings.Split(callBack, "|")
		u64, _ := strconv.ParseUint(res[1], 10, 32)
		return domainsTemplate.DetailDomainMessage(client, conn, uint(u64))
	}
	if strings.Contains(callBack, "detailTariff") {
		res := strings.Split(callBack, "|")
		u64, _ := strconv.ParseUint(res[1], 10, 32)
		return tariffsTemplate.DetailTariffMessage(client, conn, uint(u64))
	}

	if strings.Contains(callBack, "changeLinkDestination") {
		res := strings.Split(callBack, "|")
		u64, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.ChangeLinkDestinationMessage(client, conn, uint(u64))
	}

	if strings.Contains(callBack, "save_destination_link") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.SaveLinkDestinationMessage(client, conn, message, uint(urlId))
	}

	if strings.Contains(callBack, "deleteLink") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.DeleteLinkMessage(client, conn, uint(urlId))
	}

	if strings.Contains(callBack, "confirmDeleteLink") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.ConfirmDeleteLinkMessage(client, conn, uint(urlId))
	}

	if strings.Contains(callBack, "confirmDeleteDomain") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return domainsTemplate.ConfirmDeleteDomainMessage(client, conn, uint(urlId))
	}

	if strings.Contains(callBack, "changeComment") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.ChangeLinkCommentMessage(client, conn, uint(urlId))
	}

	if strings.Contains(callBack, "save_comment_link") {
		res := strings.Split(callBack, "|")
		urlId, _ := strconv.ParseUint(res[1], 10, 32)
		return linksTemplates.SaveInputCommentShotLinkMessage(client, conn, message, uint(urlId))
	}

	if strings.Contains(callBack, "deleteDomain") {
		res := strings.Split(callBack, "|")
		domainId, _ := strconv.ParseUint(res[1], 10, 32)
		return domainsTemplate.DeleteDomainMessage(client, conn, uint(domainId))
	}

	if strings.Contains(callBack, "tariffConnect") {
		res := strings.Split(callBack, "|")
		tariffId, _ := strconv.ParseUint(res[1], 10, 32)
		return tariffsTemplate.TariffConnectMessage(client, conn, uint(tariffId))
	}

	if strings.Contains(callBack, "buyTariff") {
		res := strings.Split(callBack, "|")
		tariffId, _ := strconv.ParseUint(res[1], 10, 32)
		return tariffsTemplate.BuyTariff(client, conn, uint(tariffId))
	}
